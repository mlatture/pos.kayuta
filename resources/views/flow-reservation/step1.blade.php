                ...
                <div class="card-body p-0">
                    <div class="table-responsive">
                        <table class="table table-hover mb-0" id="resultsTable">
                            ...
                            <tbody>
                                <tr>
                                    <td colspan="5" class="text-center py-5 text-muted">Enter search criteria above to see available sites.</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
            ...
            <div class="card shadow-sm cart-panel">
                ...
                <div class="card-body">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control form-control-sm" id="instantDiscount" step="0.01" value="0.00">
                    </div>
    ...
    <div class="mt-2 small">
        ...
        <select class="form-select form-select-sm add-on-select">
            ...
            @foreach($addons as $addon)
                <option value="{{ $addon->id }}" data-price="{{ $addon->price }}">{{ $addon->name }} (${{ number_format($addon->price, 2) }})</option>
            @endforeach
        </select>
...
$(function() {
    ...
    $('#searchForm').on('submit', function(e) {
        ...
        $.get("{{ route('flow-reservation.search') }}", $(this).serialize())
            .done(function(res) {
                ...
                if (units.length === 0) {
                    tbody.append('<tr><td colspan="5" class="text-center py-5">No available sites found for these criteria.</td></tr>');
                } else {
                    units.forEach(unit => {
    ...
    $('#applyCoupon').on('click', function() {
        ...
        if (code) {
            alert('Coupon logic would be implemented here. For this draft, the coupon record will be created at the next stage.');
        }
    });
});
</script>
