            ...
            <form action="{{ route('admins.update',$admin->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input type="email" name="email" class="form-control @error('email') is-invalid @enderror"
                           id="email" placeholder="Enter your email" value="{{ old('email',$admin->email) }}" required>
                    @error('email')
                    <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
                ...
                <div class="form-group">
                    ...
                    <input type="password" name="password" class="form-control @error('password') is-invalid @enderror"
                           id="password" placeholder="Enter your password" value="{{ old('password') }}" >
                    @error('password')
                    <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" name="admin_role_id" id="admin_role_id" required>
                        ...
                        @foreach($adminRoles as $adminRole)
                            <option {{ ($admin->admin_role_id == $adminRole->id) ? "selected" : "" }} value="{{ $adminRole->id }}">{{ $adminRole->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group">
                    ...
                    <select name="status" id="status" class="form-control" required>
                        ...
                        <option {{ ($admin->status == 0) ? "selected" : "" }} value="0">Inactive</option>
                    </select>
                    @error('status')
                    <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
