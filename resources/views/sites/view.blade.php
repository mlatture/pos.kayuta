            ...
            <div class="container">
                <div class="row">
                    ...
                    <div class="col-md-6">
                        ...
                        @php
                            $rigtypes = is_array($site->rigtypes)
                                ? $site->rigtypes
                                : json_decode($site->rigtypes, true);
                        @endphp
                        @if (!empty($rigtypes))
                        ...
                        @php
                            $amenities = is_array($site->amenities)
                                ? $site->amenities
                                : json_decode($site->amenities, true);
                        @endphp
                        @if (!empty($amenities))
                            <div class="d-flex flex-wrap gap-2">
                ...
                <div class="row">
                        @php
                            ...
                            $images = is_string($site->images) ? json_decode($site->images, true) : [];
                        ...
                        @if (!empty($images) && is_array($images))
                            @foreach ($images as $image)
                                <div class="col-md-3 mb-3">
                                    <img src="{{ asset('storage/' . $image) }}" class="img-fluid rounded shadow"
                                        alt="Site Image">
                                </div>
                            @endforeach
                        @else
