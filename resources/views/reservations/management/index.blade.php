    ...
    <div class="container-fluid py-3">
        <div class="d-flex align-items-center justify-content-between mb-3">
            ...
            <div class="d-flex gap-2">
                ...
                <button class="btn btn-outline-primary btn-sm" id="btnOpenCustomer">Select/Create Customer</button>
            </div>
        </div>
    ...
    <script>
        (function() {
                ...
                $.post(routes.availability, $form.serialize())
                    .done(res => {
                        ...
                        const rows = res.items.map(item => {
                            ...
                            <td class="text-end"><button class="btn btn-sm btn-outline-primary btnAdd">Add to Cart</button></td>
                        </tr>`;
                        }).join('');
                        $tbody.html(rows);
                    })
            ...
            $('#customerSearch').on('input', debounce(function() {
                ...
                if (q.length < 2) return $('#customerResults').empty();

                $.post(routes.custSearch, {
                        _token: $('input[name=_token]').val(),
                        q
                    })
                    .done(res => {
                        const html = (res.hits || []).map(h => `
          <button type="button" class="list-group-item list-group-item-action selCustomer" data-id="${h.id}">
            ${h.name} ${h.email ? ' • ' + h.email : ''} ${h.phone ? ' • ' + h.phone : ''}
          </button>`).join('');
