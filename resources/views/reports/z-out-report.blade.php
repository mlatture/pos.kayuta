...
@section('js')
    ...
    <script>
        $(document).ready(function() {
            let  usersData = [];
            let currentRegisterData = JSON.parse(localStorage.getItem("current_register_data")) || [];
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    response.forEach(res => {
                        let stationName = res.name.toUpperCase();
                        let option = `<option value="${res.id}" data-name="${stationName}">${stationName}</option>`;
                        $('#station').append(option);
                    })
            ...
            $('#station').on('change', function () {
                ...
                $('#user').append('<option disabled selected>Select a user</option >');
                    ...
                    let filteredUsers = usersData.filter(user => {
                        return currentRegisterData.some(entry =>
                            entry.current_register_id == selectedStationId &&
                            entry.current_register_staff_id == user.id
                        );
                    });

                    if (filteredUsers.length === 0) {
                        $('#user').append('<option disabled>No users available for this station</option>');
                    } else {
                        filteredUsers.forEach(user => {
                            let name = user.name.charAt(0).toUpperCase() + user.name.slice(1);
                            let option = `<option value="${user.id}" data-name="${name}">${name}</option>`;
                            $('#user').append(option);
                        });
                    }
            });
