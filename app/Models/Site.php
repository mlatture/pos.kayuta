...
class Site extends Model
{
    ...
    public function checkAllSites($cid, $cod, $siteId = null)
    {
        return $this->select([
            ...
            'class',
            'coordinates',
            DB::raw("COALESCE(cart_reservations.siteid, 'Available') AS incart"),
            DB::raw("COALESCE(reservations.siteid, 'Available') AS reserved"),
            DB::raw("COALESCE(cart_reservations.cartid, '') AS cartid")
        ])
            ->leftJoin('reservations', function ($join) use ($cid, $cod) {
                $join->on('sites.siteid', '=', 'reservations.siteid')
                    ->where(function ($query) use ($cid, $cod) {
    ...
    public function flexibleSites($cid, $cod, $riglen, $siteclass)
    {
        ...
        WHERE
        DATE_ADD('" . $cid . "', INTERVAL n DAY) <= '" . $cod . "') d")
            )
            ->leftJoin('reservations as r', function ($join) {
                $join->on('sites.siteid', '=', 'r.siteid')
                    ->whereRaw('r.cid < DATE_ADD(d.date, INTERVAL 1 DAY)')
                    ->whereRaw('r.cod > d.date');
            })
            ->leftJoin('cart_reservations as c', function ($join) {
                $join->on('sites.siteid', '=', 'c.siteid')
                    ->whereRaw('c.cid <= DATE_ADD(d.date, INTERVAL 1 DAY)')
                    ->whereRaw('c.cod >= d.date');
            })
            ...
            ->whereNotIn('siteid', function ($query) use ($cid, $cod, $cit, $cot) {
                $query->select('siteid')
                    ...
                    ->where(function ($query) use ($cid, $cod, $cit, $cot) {
                        $query->whereRaw("CONCAT('$cid', '$cit') between cid and cod")
                            ->orWhereRaw("CONCAT('$cod', '$cot') between cid and cod")
                            ->orWhere(function ($query) use ($cid, $cod, $cit, $cot) {
                                $query->where('cid', '>=', DB::raw("CONCAT('$cid', '$cit')"))
                                    ->where('cod', '<=', DB::raw("CONCAT('$cod', '$cot')"));
                            });
                    });
            })
            ->first();
