...
class Helpers
{
    ...
    public static function uploadsLiabilityDir()
    {
    ...
    public static function format_currency_usd($amount)
    {
        // Use the number_format() function to add commas as thousand separators
        $formatted_amount = number_format($amount, 2);

        // Add the dollar sign and return the result
    ...
    public static function getHeaderTable($cid, $cod, $lengthofStay)
    {
        $tablearray = [];
        $datearray = [];
        $headerrow = '';
        ...
        if ($lengthofStay > 0) {
            if ($workingDate == '') {
                $workingDate = date_create($cid);
            }

            //echo "<tr><td>" . $lengthofStay . " Nights</td></tr><tr>";
            for ($n = 1; $n <= $lengthofStay; $n++) {
                $tablearray[0][$n] = date_format($workingDate, 'D, M d');
                $datearray[$n] = date_format($workingDate, 'Y-m-d');
                //echo $n. " - ". $datearray[$n] ;
                $workingDate = date_add($workingDate, date_interval_create_from_date_string('+1 day'));
                $headerrow .= "<th  scope='col'>" . $tablearray[0][$n] . '</th>';
            }
        }
    ...
    public static function isValidColorCode($colorCode)
    {
        $pattern = '/^#[a-fA-F0-9]{6}$/';
        return preg_match($pattern, $colorCode) === 1;
    }
