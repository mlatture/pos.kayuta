    ...
    public function run()
    {
        ...
        if (File::exists($path)) {
            ...
            if (preg_match('/INSERT INTO `categories` .* VALUES\s*\(([^)]+)\);/', $sql, $matches)) {
                if (trim($matches[1]) !== '') {
                    DB::unprepared($sql);
                    $this->command->info('Categories data seeded from categories.sql');
                } else {
                    $this->command->info('No data to insert into categories table. Skipping...');
                }
            } else {
                $this->command->info('No INSERT statement found in categories.sql. Skipping...');
            }
        } else {
