...
class CampsitePicsSeeder extends Seeder
{
    ...
    public function run()
    {
       ...
       if (!File::exists($path)) {
        ...
        if (!preg_match('/INSERT INTO `campsite_pics` \(([^)]+)\) VALUES/is', $sql, $columnsMatch)) {
        ...
        foreach ($rows[1] as $row) {
            ...
            $exists = DB::table('campsite_pics')
                ->where('title', $data['title'] ?? null)
                ->where('created_at', $data['created_at'] ?? null)
                ->exists();

            if (!$exists) {
                DB::table('campsite_pics')->insert($data);
            }
        }

        $this->command->info('Camp Site Pics table seeded successfully!');
    }
}
