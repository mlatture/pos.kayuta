...
class LocalAreaAttractionsSeeder extends Seeder
{
    ...
    public function run()
    {
        $path = database_path('seeders/sql/local_area_attractions.sql');

        if (File::exists($path)) {
            $sql = File::get($path);

            if (preg_match('/INSERT INTO `local_area_attractions` .* VALUES\s*\(([^)]+)\);/', $sql, $matches)) {
                if (trim($matches[1]) !== '') {
                    DB::unprepared($sql);
                    $this->command->info('Local Area Attractions data seeded from local_area_attractions.sql');
                } else {
                    $this->command->info('No data to insert into local_area_attractions table. Skipping...');
                }
            } else {
                $this->command->info('No INSERT statement found in local_area_attractions.sql. Skipping...');
            }
        } else {
