...
class ReceiptsTableSeeder extends Seeder
{
    ...
    public function run()
    {
        $path = database_path('seeders/sql/receipts.sql');

        if (File::exists($path)) {
            $sql = File::get($path);

            if (preg_match('/INSERT INTO `receipts` .* VALUES\s*\(([^)]+)\);/', $sql, $matches)) {
                if (trim($matches[1]) !== '') {
                    DB::unprepared($sql);
                    $this->command->info('Receipts data seeded from receipts.sql');
                } else {
                    $this->command->info('No data to insert into receipts table. Skipping...');
                }
            } else {
                $this->command->info('No INSERT statement found in receipts.sql. Skipping...');
            }
        } else {
