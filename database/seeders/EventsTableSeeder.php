...
class EventsTableSeeder extends Seeder
{
    ...
    public function run()
    {
        $path = database_path('seeders/sql/events.sql');

        if (File::exists($path)) {
            $sql = File::get($path);

            if (preg_match('/INSERT INTO `events` .* VALUES\s*\(([^)]+)\);/', $sql, $matches)) {
                if (trim($matches[1]) !== '') {
                    DB::unprepared($sql);
                    $this->command->info('Events data seeded from events.sql');
                } else {
                    $this->command->info('No data to insert into events table. Skipping...');
                }
            } else {
                $this->command->info('No INSERT statement found in events.sql. Skipping...');
            }
        } else {
