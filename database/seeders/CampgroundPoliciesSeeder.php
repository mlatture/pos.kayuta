...
class CampgroundPoliciesSeeder extends Seeder
{
    /**
    ...
    public function run()
    {
        $path = database_path('seeders/sql/campground_policies.sql');
        ...
        if (!preg_match('/INSERT INTO `campground_policies` \(([^)]+)\) VALUES/is', $sql, $columnsMatch)) {
        ...
        foreach ($rows[1] as $row) {
            ...
            $exists = DB::table('campground_policies')
                ->where('title', $data['title'] ?? null)
                ->where('created_at', $data['created_at'] ?? null)
                ->exists();

            if (!$exists) {
                DB::table('campground_policies')->insert($data);
            }
        }

        $this->command->info('Campground Policies table seeded successfully!');
    }
}
