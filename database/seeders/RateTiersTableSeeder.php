...
class RateTiersTableSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        if (File::exists($path)) {
            ...
            if (preg_match('/INSERT INTO `rate_tiers` .* VALUES\s*\(([^)]+)\);/', $sql, $matches)) {
                if (trim($matches[1]) !== '') {
                    DB::unprepared($sql);
                    $this->command->info('Rate Tiers data seeded from rate_tiers.sql');
                } else {
                    $this->command->info('No data to insert into rate_tiers table. Skipping...');
                }
            } else {
                $this->command->info('No INSERT statement found in rate_tiers.sql. Skipping...');
            }
        } else {
