...
class AdminsSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        if (File::exists($path)) {
            ...
            if (preg_match('/INSERT INTO `admins` .* VALUES\s*\(([^)]+)\);/', $sql, $matches)) {
                if (trim($matches[1]) !== '') {
                    DB::unprepared($sql);
                    $this->command->info('Admins data seeded from admins.sql');
                } else {
                    $this->command->info('No data to insert into admins table. Skipping...');
                }
            } else {
                $this->command->info('No INSERT statement found in admins.sql. Skipping...');
            }
        } else {
